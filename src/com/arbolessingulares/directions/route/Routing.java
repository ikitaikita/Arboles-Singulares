...
public class Routing extends AsyncTask<LatLng, Void, Route> {
    ...
    public Routing(TravelMode mTravelMode) {
        ...
        Log.i("mTravelMode.getValue(): ", mTravelMode.getValue());
        if(mTravelMode.getValue().equals("walking"))this.color = Color.BLUE;
        if(mTravelMode.getValue().equals("biking"))this.color = Color.GREEN;
        if(mTravelMode.getValue().equals("driving"))this.color = Color.GRAY;
    }
    ...
    protected void dispatchOnSuccess(PolylineOptions mOptions) {
        ...
        for (RoutingListener mListener : _aListeners) {
        	Log.i("devolver TRAVEL MODE: ", this._mTravelMode.getValue());
        	 
            mListener.onRoutingSuccess(mOptions.color(this.color));
        }
    }
